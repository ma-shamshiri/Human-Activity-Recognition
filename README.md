...
<p align="justify"> 
  The WISDM (Wireless Sensor Data Mining) dataset includes raw time-series data collected from accelerometer and gyroscope sensors of a smartphone and smartwatch with their corresponding labels for each activity. The sensor data was collected at a rate of 20 Hz (i.e., every 50ms). Weiss et.al., collected this dataset from 51 subjects who performed 18 different activities listed in Table 2, each for 3 minutes, while having the smartphone in their right pant pocket and wearing the smartwatch in their dominant hand. <br>
  In this project we tried three different feature sets, extracted from the raw data, which are 1.Pre-processed data generated by Weiss et.al 2.Statistical feature extraction and 3.Topological feature extraction. All these three approaches used windowing technique to segment the raw time series
and extract features from each segment. <br>
